...
- VERIFY platform compatibility: path.join() used for all paths, no hardcoded separators
 - GRAPH-FIRST REVIEW: call \`repo_map action="graph_health"\`, then use \`repo_map action="symbol_context"\` for definition-first verification and \`diff_context\`/\`impact_cone\` (or \`blast_radius\` for file-level dependents) to localize the changed surface. Use \`repo_map action="dead_exports" top_n=100\` only as advisory review candidates, never deletion proof. Require source anchors, provenance, and direct-source verification. Graph evidence is advisory only, never an approval or denial; if freshness is stale or inconclusive, confidence is low, source is missing, the language is unsupported/dynamic, the graph is absent, or an action fails, fall back to the direct source, Git diff, and searches.
