...
export const cancel_lane_batch: ReturnType<typeof createSwarmTool> =
	createSwarmTool({
		description:
			'Cancel ACTIVE lanes of a dispatch_lanes_async batch — the ONLY cancellation surface; collect_lane_results cannot cancel. DESTRUCTIVE and authorization-gated: requires confirm: true plus a bounded reason, recorded on every cancelled lane terminal as the distinct operator_cancelled failure class (never liveness). Race-safe per lane: a fresh liveness snapshot REFUSES busy/retry lanes (live work is never destroyed here — the human force path is /swarm abort-pr-workflow); a degraded/absent status probe refuses the whole batch fail-closed; a session the host does not know or reports idle may be cancelled; a child completing between preflight and abort wins; an abort timeout never claims cancellation. Terminal and exactly-once; operator-cancelled dimensions are not auto-retryable and never consume the retry budget — re-dispatch explicitly.',
		args: {
