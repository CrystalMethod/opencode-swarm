...
const CATALOG_SOURCE: readonly (readonly [string, CatalogEntryInput])[] = [
	...
	[
		'verdict_row_pipe_recovery',
		{
			...
			severity: 'info',
			privacyClass: 'pseudonymous',
			producer: 'src/pr-review/legacy-transcript-adapter.ts:689',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_LIFECYCLE_RETENTION,
	...
	[
		...
		{
			// #2486 (D7): counts-only health for the consented training vault.
			// Emitted on capture-stop transitions (quota/disk/consent), on
			// withdrawals, and available for quarantine aggregates. Payload is
			// a pseudonymous project ref, a reason enum, and a count — no vault
			// content, no raw paths — so the metadata-only observability
			// contract holds for the governed-content subsystem too.
			category: 'guardrail',
			severity: 'notice',
			privacyClass: 'pseudonymous',
			producer: 'src/telemetry.ts:1167',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_LIFECYCLE_RETENTION,
	...
	[
		'retrieval_routed',
		{
			...
			severity: 'info',
			privacyClass: 'pseudonymous',
			producer: 'src/telemetry.ts:1400',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_SINK,
			requiredWorkflowIds: REQUIRE_SESSION,
	...
	[
		'prm_hard_stop_delivered',
		{
			...
			privacyClass: 'pseudonymous',
			producer: 'src/telemetry.ts:1381',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_SINK,
			requiredWorkflowIds: REQUIRE_SESSION,
	...
	[
		'prm_pattern_detected',
		{
			...
			severity: 'notice',
			privacyClass: 'pseudonymous',
			producer: 'src/telemetry.ts:1013',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_SINK,
			requiredWorkflowIds: REQUIRE_SESSION,
	...
	[
		'prm_course_correction_injected',
		{
			...
			severity: 'notice',
			privacyClass: 'pseudonymous',
			producer: 'src/telemetry.ts:1027',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_SINK,
			requiredWorkflowIds: REQUIRE_SESSION,
	...
	[
		'prm_escalation_triggered',
		{
			...
			severity: 'warning',
			privacyClass: 'pseudonymous',
			producer: 'src/telemetry.ts:1040',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_SINK,
			requiredWorkflowIds: REQUIRE_SESSION,
	...
	[
		'prm_hard_stop',
		{
			...
			severity: 'critical',
			privacyClass: 'pseudonymous',
			producer: 'src/telemetry.ts:1054',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_SINK,
			requiredWorkflowIds: REQUIRE_SESSION,
	...
	[
		...
		{
			// Issue #2678 — TERMINAL/handoff transition of a PRM hard-stop
			// episode; the third noninterchangeable counter after the TRIGGER
			// (`prm_hard_stop`) and DELIVERY (`prm_hard_stop_delivered`).
			category: 'prm',
			severity: 'critical',
			privacyClass: 'pseudonymous',
			producer: 'src/telemetry.ts:1079',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_SINK,
			requiredWorkflowIds: REQUIRE_SESSION,
	...
	[
		'close_archive_result',
		{
			...
			severity: 'notice',
			privacyClass: 'operational',
			producer: 'src/telemetry.ts:1116',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_SINK,
		},
	...
	[
		'residue_health',
		{
			...
			severity: 'notice',
			privacyClass: 'operational',
			producer: 'src/telemetry.ts:1138',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_SINK,
	...
	[
		'context_telemetry_health',
		{
			...
			severity: 'notice',
			privacyClass: 'operational',
			producer: 'src/telemetry.ts:1182',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_SINK,
			// Aggregate-only counts; no workflow correlation applies.
	...
	[
		'skill_usage_health',
		{
			...
			severity: 'notice',
			privacyClass: 'operational',
			producer: 'src/telemetry.ts:1219',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_SINK,
			// Aggregate-only counts; no workflow correlation applies.
	...
	[
		'core_events_health',
		{
			...
			severity: 'notice',
			privacyClass: 'operational',
			producer: 'src/telemetry.ts:1245',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_SINK,
			// Aggregate-only counts; no workflow correlation applies.
	...
	[
		'shell_audit_health',
		{
			...
			severity: 'notice',
			privacyClass: 'operational',
			producer: 'src/telemetry.ts:1269',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_SINK,
			// Aggregate-only counts; no workflow correlation applies.
	...
	[
		'trajectory_health',
		{
			...
			severity: 'notice',
			privacyClass: 'operational',
			producer: 'src/telemetry.ts:1292',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_SINK,
			// Aggregate-only counts; no workflow correlation applies.
	...
	[
		'pr_subscription_health',
		{
			...
			severity: 'notice',
			privacyClass: 'operational',
			producer: 'src/telemetry.ts:1321',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
			retentionOwnerIssue: ISSUE_SINK,
			// Aggregate-only counts; no workflow correlation applies.
	...
	[
		'learning_health_alarm',
		{
			...
			severity: 'warning',
			privacyClass: 'operational',
			producer: 'src/telemetry.ts:1365',
			consumers: NO_CONSUMERS,
			futureOwnerIssue: ISSUE_SINK,
	...
	[
		...
		{
			// Issue #2676 (Workstream D16): ONE event per execution attempt,
			// bound to the exact task/call/invocation/generation identity the
			// producer holds, with a closed attempt-class vocabulary (denial /
			// attempt / result / duplicate / late / cancelled / provider_failed),
			// explicit capture coverage (`captured` / `unknown` field lists), and
			// a per-attempt cost block whose unknown axes are strictly `null`
			// plus an `unavailable` list entry — never 0. Producers: the
			// delegation lifecycle (begin/terminal) and the Stage A gate route
			// hook (denial / result / late / duplicate classes). Live reader: the
			// /swarm report task-attempts cohort fold.
			category: 'lifecycle',
			severity: 'info',
			privacyClass: 'pseudonymous',
			producer: 'src/services/execution-attempt.ts:265',
			consumers: ['src/commands/report.ts:60'],
			retentionOwnerIssue: 2676,
			requiredWorkflowIds: REQUIRE_SESSION,
			forbiddenWorkflowIds: FORBID_COUNCIL_ROUND,
			otelMapping: 'none',
		},
