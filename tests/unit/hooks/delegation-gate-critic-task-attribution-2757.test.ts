...
describe('delegation-gate — regression: plan-level critic attribution (#2757)', () => {
	...
	it('does not let a named ID shadow a valid TASK marker (F3)', async () => {
		const session = ensureAgentSession(
			'critic-mixed-attribution',
			'architect',
			tmpDir,
		);
		const resolved = await delegationGateInternals.resolveEvidenceTaskId(
			{
			...
			session,
			tmpDir,
			{ policy: 'attribution', allowSessionFallback: false },
		);

		expect(resolved).toBe(TASK_ID);
	});
