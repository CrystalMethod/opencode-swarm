...
8. **Anti-tampering.** Once the Phase 2.5 red checkpoint is frozen, weakening, skipping, or deleting an acceptance check is a contract violation; any legitimate change goes through the checkpoint manifest as an `AMEND` row with a closed reason. `repro-check.sh` enforces the manifest shape and supports an externally published `anchor` receipt: a reviewer records the exact `issue-tracer-checkpoint-v1` line after Phase 2.5, outside the writable trace, and later runs `verify-anchor` against that literal. The receipt binds the no-filter manifest blob hash and the `checkpoint-tree-id` recorded in `state.md`; it deliberately does not compare the live working-tree tree, which changes during implementation. Editing a blob id in place or deleting/refreezing the manifest therefore fails against the published receipt. A byte-identical refreeze remains valid by design. The manifest is still evidence a reviewer re-runs and reads; the external receipt is the non-circular identity anchor.
