...
export const RETENTION_REGISTRY: readonly RetentionRow[] = [
	...
	},
	{
		...
		writerCitations: ['src/background/pr-feedback-event-queue.ts:455 writeQueueRecord — atomic temp+fsync+Windows-retry rename (enqueue/claim)'],
		readerCitations: ['src/background/pr-feedback-event-queue.ts:723 readPrFeedbackMonitorQueueFromDisk — bounded ≤512 KiB with identity verification, async'],
		...
		readBound: { pattern: 'indexed', bound: '≤512 KiB hard read bound', sync: false, citation: 'src/background/pr-feedback-event-queue.ts:441-471' },
	...
	},
	{
		id: 'live-lane-owners',
		category: 2,
		pathGrammar: '.swarm/live-lane-owners.json',
		canonicalRoot: 'project-swarm',
		writerModules: ['src/parallel/lane-owners.ts'],
		writerCitations: [
			'src/parallel/lane-owners.ts recordLiveLaneOwner — dedupe-by-canonical-lanePath merge, then writeStore rewrites the bounded store atomically via atomicWriteSwarmFileSync; the only writers are the lean runner post-provision and the standard worktree-isolation post-provision paths (issue #2527 F3)',
			'src/parallel/lane-owners.ts clearLiveLaneOwner — removes one record; called from removeWorktree (src/worktree/core.ts, both success returns)',
		],
		readerCitations: [
			'src/parallel/lane-owners.ts listLiveLaneOwners — single bounded read plus GC-on-read: records whose lane path no longer exists or whose startedAt is past the 24h claim window are reaped and the store rewritten',
			'src/hooks/init-orphan-recovery.ts liveLaneOwners scan (after the fail-closed uncertainty guards, before migration and enumeration) — adds live-owned lanePaths to protectedWorktreePaths',
		],
		schemaVersion: '1 (LiveLaneOwnerStore.schemaVersion, lane-owners.ts)',
		stateClass: 'operational',
		privacyClass: 'metadata',
		writeLimits: {
			bound: 'MAX_LIVE_LANE_OWNERS=256 entries + MAX_LIVE_LANE_OWNERS_BYTES=128 KiB serialized, drop-oldest enforced on BOTH axes in writeStore',
			scope: 'global',
			citation: 'src/parallel/lane-owners.ts writeStore',
		},
		readBound: {
			pattern: 'full-file',
			bound: 'single bounded JSON read per call; oversized/malformed reads fail to the empty store',
			sync: true,
			citation: 'src/parallel/lane-owners.ts readStore',
		},
		lockModel: 'in-process; atomic temp+rename last-writer-wins — a raced record at worst drops one lane from liveness protection for one pass; GC-on-read plus the PID claim window make replays idempotent',
		crashBehavior: 'atomic temp+rename; a crashed host leaves records whose startedAt ages past the 24h claim window and are reaped on the next read — no state is only-recoverable from this file',
		closePolicy: 'untouched — live lanes must survive /swarm close; records are cleared when their worktrees are removed (removeWorktree) or reaped by GC',
		closeArrayMembership: {
			'live-lane-owners.json': 'neither',
		},
		resetPolicy: 'not reset — /swarm reset-session treats live-owned lanes as confirm-required candidates and never blanket-clears the store',
		legacyCompatibility: 'n/a — new in #2527; file absence reads as the empty store',
		healthSignal: 'no dedicated advisory; lanes left at legacy paths by migration are surfaced by the init pass recovery warnings',
		owner: '#2527',
		disposition: {
			kind: 'not-a-defect',
			proof: 'Hard dual bound 256 entries / 128 KiB serialized with drop-oldest on both axes (src/parallel/lane-owners.ts writeStore); the store is a live-liveness signal whose records are cleared on worktree removal and GC-reaped on read — bounded by construction (issue #2527).',
		},
	},
	{
		id: 'pending-purge',
		category: 2,
		pathGrammar: '.swarm/pending-purge.json',
		canonicalRoot: 'project-swarm',
		writerModules: ['src/commands/destructive-purge.ts'],
		writerCitations: [
			'src/commands/destructive-purge.ts issueConfirmToken — single-slot pending record {scopeDigest, confirmToken, createdAt} written atomically; every new issuance overwrites the previous (the old token stops working)',
			'src/commands/destructive-purge.ts executeDestructivePurge — consumes the record on any execution attempt outcome',
		],
		readerCitations: [
			'src/commands/destructive-purge.ts readPending — single bounded read; a record past the 15-minute TTL reads as absent (silent expiry)',
			'src/commands/reset-session.ts confirmed purge path — validates token AND re-derived current scope digest before destructive execution',
		],
		schemaVersion: '1 (PendingPurgeRecord.schemaVersion, destructive-purge.ts)',
		stateClass: 'operational',
		privacyClass: 'metadata',
		writeLimits: {
			bound: 'single fixed-shape record (scope digest + token + createdAt), 15-minute TTL, single slot — no accumulation axis exists',
			scope: 'global',
			citation: 'src/commands/destructive-purge.ts issueConfirmToken/executeDestructivePurge',
		},
		readBound: {
			pattern: 'full-file',
			bound: 'single small JSON read per validation',
			sync: true,
			citation: 'src/commands/destructive-purge.ts readPending',
		},
		lockModel: 'in-process; atomic temp+rename single-slot overwrite — a raced issuance invalidates the older token, which is the designed semantics (no pending purge for this scope)',
		crashBehavior: 'atomic temp+rename; a stale record simply expires after the TTL — destructive execution without a fresh confirm is impossible',
		closePolicy: 'untouched — a pending purge confirmation dying with the session is the safe direction',
		closeArrayMembership: {
			'pending-purge.json': 'neither',
		},
		resetPolicy: 'not reset — /swarm reset-session is a CONSUMER (issues/validates tokens), never a blanket clear',
		legacyCompatibility: 'n/a — new in #2527; file absence reads as no pending purge',
		healthSignal: 'the command output itself (preview lines + --confirm=<token> instruction); no separate advisory',
		owner: '#2527',
		disposition: {
			kind: 'not-a-defect',
			proof: 'Single-slot fixed-shape record with a 15-minute TTL and consume-on-execute (src/commands/destructive-purge.ts) — no growth axis and no destructive reach without a fresh two-step confirmation (issue #2527).',
		},
	},
	{
		id: 'dead-lane-reclaims',
		category: 2,
		pathGrammar: '.swarm/dead-lane-reclaims.json',
		canonicalRoot: 'project-swarm',
		writerModules: ['src/hooks/delegation-gate/dead-lane-reclaim.ts'],
		writerCitations: [
			'src/hooks/delegation-gate/dead-lane-reclaim.ts:139 recordDeadLaneReclaim — dedupe-by-lanePath merge, then writeStore (:112) rewrites the bounded store atomically via atomicWriteSwarmFileSync (:131); the only writer — the three strand sites (worktree-isolation handleStrandedLane, delegation-gate terminal-failure path, reset-session EBUSY bulk-clean path) all call this one seam (issue #2599)',
			'src/hooks/delegation-gate/dead-lane-reclaim.ts:250 reclaimDeadLanes — the consuming pass at next start; fs.rmSync force removes the store file once no entries remain (:329)',
		],
		readerCitations: [
			'src/hooks/delegation-gate/dead-lane-reclaim.ts:89 readStore — single file, sync readFileSync bounded by MAX_DEAD_LANE_RECLAIM_BYTES (:91-93); missing/oversized/malformed reads fail to the empty store, never throw',
			'src/hooks/init-orphan-recovery.ts reclaimDeadLanes (Step 3) — withTimeout-bounded startup pass; retained/preserved outcomes logged, never fatal to init',
		],
		schemaVersion: '1 (DeadLaneReclaimStore.schemaVersion, dead-lane-reclaim.ts:52)',
		stateClass: 'operational',
		privacyClass: 'metadata',
		writeLimits: {
			bound: 'MAX_DEAD_LANE_RECLAIMS=512 entries + MAX_DEAD_LANE_RECLAIM_BYTES=256 KiB serialized, drop-oldest enforced on BOTH axes in writeStore (dead-lane-reclaim.ts:35-36,112-134)',
			scope: 'global',
			citation: 'src/hooks/delegation-gate/dead-lane-reclaim.ts:35-36,116-134',
		},
		readBound: {
			pattern: 'full-file',
			bound: 'single ≤256 KiB JSON, read once per recordDeadLaneReclaim and once per startup reclaim pass',
			sync: true,
			citation: 'src/hooks/delegation-gate/dead-lane-reclaim.ts:89-110',
		},
		lockModel: 'in-process; atomic temp+rename last-writer-wins rewrite — a raced record at worst drops one lane from a later reclaim pass, and replay is fail-closed (ownership + dirty gates make every reclaim idempotent)',
		crashBehavior: 'atomic temp+rename; a torn read fails to the empty store and the stranded lane directory itself remains visible to init-orphan-recovery\u2019s orphan sweep — no state is only-recoverable from this file',
		closePolicy: 'untouched — a pending record must survive /swarm close so the next start\u2019s init-orphan-recovery reclaims the stranded lane; the store is the issue #2599 recovery mechanism, not session output',
		closeArrayMembership: {
			'dead-lane-reclaims.json': 'neither',
		},
		resetPolicy: 'not reset — /swarm reset-session APPENDS strand records for lanes it could not bulk-delete (src/commands/reset-session.ts EBUSY path); the next start consumes them',
		legacyCompatibility: 'n/a — new in #2599; file absence reads as an empty store',
		healthSignal: 'WORKTREE_LANE_STRANDED advisory (delegation-gate / worktree-isolation / reset-session) + "Preserved dead lane …" startup warnings (init-orphan-recovery); both surface the strand reason (may include raw git/OS error text; never provider or tool output)',
		owner: '#2599',
		disposition: {
			kind: 'not-a-defect',
			proof: 'Hard dual bound 512 entries / 256 KiB serialized with drop-oldest on both axes (src/hooks/delegation-gate/dead-lane-reclaim.ts:35-36,116-134); the store is a transient pending-reclaim queue — entries are consumed at the next start and the file deleted when empty (:250,:329), with fail-closed ownership/dirty gates preventing destructive replay (issue #2599 acceptance).',
		},
	},
	{
	...
	},
	{
		...
		readerCitations: [
			...
			'src/review/routing-enforcement.ts:516 readReviewRouteReceipt — bounded regular-file read, ≤64 KiB',
			'src/review/routing-enforcement.ts:589 readReviewRouteReceiptSync — bounded regular-file read, ≤64 KiB',
		],
	...
	},
	{
		...
		category: 2,
		pathGrammar:
			'.swarm/pr-workflow-gates/{session-stem}.json (+ .lock, .imported, .sqlite-projection) + SQLite project-database coordination_state[pr-workflow.state:<session-stem>, state] (swarm.db authority; JSON projection; session stem is sanitized slug+digest of sessionID)',
		canonicalRoot: 'project-swarm',
		writerModules: ['src/pr-review/persistence.ts', 'src/hooks/pr-workflow-gate.ts'],
		writerCitations: [
			'src/pr-review/persistence.ts:writeStateWhileLocked — SQLite coordination authority with revision/CAS and post-commit JSON projection',
			'src/pr-review/persistence.ts:deleteStateWhileLocked — terminal coordination-state delete followed by projection delete',
			'src/hooks/pr-workflow-gate.ts:activatePrWorkflow:1661-1665 — project checkout-mutation lock before per-session state-mutation lock',
			'src/hooks/pr-workflow-gate.ts:clearPrWorkflowGateState:2234-2272 — terminal delete under per-session state-mutation lock only; no checkout lock',
		],
		readerCitations: [
			'src/pr-review/persistence.ts:readPrWorkflowGateStateFromDisk — authority-first read and JSON projection repair',
			'src/pr-review/persistence.ts:readPrWorkflowGateStateFromCoordination — coordination_state namespace reader',
			'src/pr-review/persistence.ts:importLegacyPrWorkflowGateStateIfNeeded — bounded single-file legacy import and recovery replay',
			'src/hooks/pr-workflow-gate.ts:readPrWorkflowGateStateForRecovery — crash/malformed-state recovery reader',
		],
		...
		writeLimits: { bound: 'one bounded coordination_state row and one atomic JSON projection per workflow session; projection sidecars are bounded one-for-one', scope: 'per-key', keyspaceBound: 'FINITE BY REAPER: workflow session projections and sidecars are age-pruned with the PR workflow retention family (src/retention/sweep.ts)', citation: 'src/pr-review/persistence.ts; src/retention/sweep.ts' },
		readBound: { pattern: 'indexed', bound: 'one bounded coordination row plus one bounded session projection', sync: false, citation: 'src/pr-review/persistence.ts:readPrWorkflowGateStateFromDisk/readPrWorkflowGateStateFromCoordination' },
		lockModel: 'activation: project checkout-mutation lock, then per-workflow-session state-mutation lock; clear/terminal: per-session state-mutation lock only; all persistence uses SQLite transaction/CAS revision fence',
		crashBehavior: 'SQLite WAL recovery; a coordination commit before projection/archive write is replayable and authority-first reads repair the JSON projection',
		closePolicy: 'JSON projections and sidecars are untouched by close — the 30 d retention sweep owns workflow shadow lifecycle; the SQLite authority follows the swarm.db project-db archive+clean lifecycle',
		resetPolicy: 'not reset',
		legacyCompatibility: 'legacy pr-workflow-gates JSON is imported once into coordination_state and retained as a repaired JSON projection; the .imported archive is projection history, not gate authority',
		...
		owner: '#2510 (historical implementation: #2385)',
		disposition: { kind: 'not-a-defect', proof: 'The logical stream is bounded by one SQLite authority row per workflow session plus a 30-day-pruned JSON projection/sidecars; terminal gate deletion removes authority and projection, while bounded pr_workflow_* core events remain a separate audit explanation.' },
	},
	{
		id: 'status-artifacts',
		category: 2,
	...
	},
	{
		id: 'dashboard-status',
		category: 2,
		pathGrammar: '.swarm/dashboard-status.json',
		canonicalRoot: 'project-swarm',
		writerModules: ['src/dashboard/index.ts'],
		writerCitations: [
			'src/dashboard/index.ts:82 writeStatusFile — plain writeFileSync (start/conflict/close rewrite points)',
		],
		readerCitations: ['src/commands/dashboard.ts:25 readStatusFileRecord — single file, async, fail-open null'],
		schemaVersion: 'none',
		stateClass: 'operational',
		privacyClass: 'metadata',
		writeLimits: { bound: 'single rewritten 4-field snapshot (status/port/url/startedAt; the capability token is never persisted)', scope: 'global', citation: 'src/dashboard/index.ts:82-109' },
		readBound: { pattern: 'indexed', bound: 'single small JSON', sync: false, citation: 'src/commands/dashboard.ts:25-40' },
		lockModel: 'none; fail-open (write failure → bounded log line, dashboard still runs)',
		crashBehavior: 'non-atomic writeFileSync; truncated file fail-opens to null and the next lifecycle event rewrites it',
		closePolicy: 'rewritten to status=stopped on listener close; never deleted by close',
		closeArrayMembership: {
			'dashboard-status.json': 'neither',
		},
		resetPolicy: 'not reset',
		legacyCompatibility: 'reader returns null on any parse failure',
		healthSignal: 'n/a',
		owner: '#2509',
		disposition: { kind: 'not-a-defect', proof: 'Single rewritten bounded snapshot written only on dashboard lifecycle transitions; disabled config never writes it (zero-footprint contract).' },
	},
	{
		id: 'locks-dir',
		category: 2,
	...
	},
	{
		...
		writerCitations: ['src/hooks/curator-postmortem.ts:1119 runCuratorPostMortem — atomicWriteFile :1399, advisory lock, dedup'],
		readerCitations: ['src/hooks/curator-postmortem.ts:844 isReportValid — full read for validity, sync; /swarm status renders it'],
	...
	},
	{
		...
		writerCitations: [
			'src/gate-evidence.ts:1346 transitionTaskWorkflowEvidence / :1515 recordGateEvidence / :1610 recordAgentDispatch — locked read-modify-write, atomic write',
			'src/council/council-evidence-writer.ts:96 writeCouncilEvidence — gates.council section under withTaskEvidenceLock',
		],
		readerCitations: [
			'src/gate-evidence.ts:1654 readTaskEvidence — FULL-FILE fail-open, async; :1730 readTaskEvidenceRaw — strict, sync',
	...
	},
	{
		...
		writerCitations: [
			...
			'issue-trace-state.ts:459 writeTraceState + issue.ts:361-419 — transactional trace-gated write with rollback (:375-419)',
		],
		readerCitations: ['issue-trace-state.ts:420,438 — full-file reads with legacy completed→status normalization (:374-393); v3 readers branch-freshness/trace-validation/merge-approval co-located'],
		schemaVersion: 'per-artifact Zod schemas (commands ≤200, text ≤4000 chars; v3 receipts ≤10 upserted entries, SHAs 40-hex)',
		stateClass: 'governed-content',
		privacyClass: 'metadata',
		writeLimits: { bound: 'single rewritten receipt files; bounded fields', scope: 'global', citation: 'record-issue-reproduction.ts:28-31; record-recurrence-sweep.ts:47-74; record-trace-validation.ts:50-63' },
		readBound: { pattern: 'indexed', bound: 'single small JSONs', sync: true, citation: 'src/hooks/issue-trace-state.ts:420-436' },
		lockModel: 'atomic writes via the canonical helper; trace-validation upserts serialize under a per-file receipt lock (issue #2788); issue pair is transactional with rollback',
		crashBehavior: 'canonical temp+rename everywhere with own-temp cleanup and bounded rename retry (issue #2788); fail-open reads',
	...
	},
	{
		...
		writerCitations: [
			'src/background/pr-feedback-loop.ts writeLoopState (writePrWorkflowAtomicJson rewrite, schemaVersion 1) / oversight evidence fsSync.writeFileSync / cleanup receipts',
		],
		readerCitations: [
			'src/background/pr-feedback-loop.ts readLoopState (Zod-validated, bounded by MAX_TRACKED_SESSIONS 200 FIFO)',
		],
		schemaVersion: 'schemaVersion:1 (LoopStateV1, zod-validated read)',
		stateClass: 'operational',
		privacyClass: 'metadata',
		directFileExemption: {
	...
	},
	{
		...
		category: 5,
		pathGrammar: '.swarm/speckit-checkoff-ledger.json',
		canonicalRoot: 'project-swarm',
		writerModules: ['src/sdd/speckit-checkoff.ts'],
		writerCitations: [
			'src/sdd/speckit-checkoff.ts writeLedger (atomicWriteSwarmFileSync rewrite of SPECKIT_CHECKOFF_LEDGER_REL :41-44)',
		],
		readerCitations: [
			'src/sdd/speckit-checkoff.ts readLedger (readBounded, MAX_TASKS_FILE_BYTES 512 KiB)',
		],
		schemaVersion: 'version:1 (SpeckitCheckoffLedger shape check)',
		stateClass: 'derived-rebuildable',
		privacyClass: 'content',
		directFileExemption: {
			reason: 'Derived projection state keyed by repo artifacts; never a phase-gate input; rebuildable at any time from specs/<feature>/tasks.md.',
			reviewedIssue: 2501,
		},
		writeLimits: { bound: 'single rewritten JSON, one entry per projected feature (MAX_SPEC_FILES 100) with bounded task entries', scope: 'per-trigger', citation: 'src/sdd/speckit-checkoff.ts writeSpeckitCheckoffLedger + MAX_TASKS_FILE_BYTES (one bounded ledger per projection)' },
		readBound: { pattern: 'full-file', bound: '512 KiB bounded read', sync: true, citation: 'src/sdd/speckit-checkoff.ts readBounded' },
		lockModel: 'single-process sync rewrite via the canonical atomic-write grammar',
		crashBehavior: 'temp+rename atomic; a torn write is impossible, a stale ledger self-heals on next projection',
		closeArrayMembership: { 'speckit-checkoff-ledger.json': 'neither' },
		closePolicy: 'retained (derived state; harmless across runs, rebuilt on next /swarm sdd project)',
		resetPolicy: 'not reset',
		legacyCompatibility: 'n/a',
		healthSignal: 'n/a',
		owner: 'this-gate',
		disposition: { kind: 'not-a-defect', proof: 'Single rewritten bounded JSON derived from user-owned artifacts; no append growth dimension.' },
	},
	{
	...
	},
	{
		...
		category: 6,
		pathGrammar: '.swarm/mcp-write-receipts.jsonl + .swarm/mcp-write-receipts-archive.jsonl',
		canonicalRoot: 'project-swarm',
		writerModules: ['src/mcp/write-receipts.ts'],
		writerCitations: [
			'src/mcp/write-receipts.ts:756-865 writeReceiptStorage — bounded active/archive JSONL rewrites via atomicWriteSwarmFile',
			'src/mcp/write-receipts.ts:866-906 withReceiptLock — locked receipt transition persistence',
		],
		readerCitations: [
			'src/mcp/write-receipts.ts:599-755 readJournal/readReceiptStorage — lstat-guarded fixed-buffer reads and schema validation',
			'src/mcp/write-receipts.ts:907-1032 prepareReceipt — bounded lifecycle lookup under the receipt lock',
			'src/mcp/write-receipts.ts:538-585 readWriteReceiptStatus — bounded status probe without writes',
		],
		schemaVersion: 'receipt record version 1 (Zod-validated JSONL records)',
		stateClass: 'authoritative',
		privacyClass: 'mixed',
		directFileExemption: {
			reason: 'MCP write settlement is intentionally a separate direct-file authority: PREPARED is durable before knowledge mutation, the receipt-only lock is released before production execution, and the bounded active journal plus terminal archive preserves uncertainty without nesting the knowledge-store transaction lock.',
			reviewedIssue: 2500,
		},
		writeLimits: {
			bound: 'active MAX_RECEIPT_RECORDS 500 / MAX_RECEIPT_JOURNAL_BYTES 512 KiB; terminal archive MAX_RECEIPT_ARCHIVE_RECORDS 10000 / MAX_RECEIPT_ARCHIVE_BYTES 16 MiB; MAX_RECEIPT_LINE_BYTES 16 KiB; archive pressure fails closed (:37-45,684-760)',
			scope: 'global',
			citation: 'src/mcp/write-receipts.ts:37-45,684-760',
		},
		readBound: {
			pattern: 'line-bounded',
			bound: 'active ≤512 KiB/500 records plus archive ≤16 MiB/10000 records; fixed-buffer reads before parsing',
			sync: false,
			citation: 'src/mcp/write-receipts.ts:587-684',
		},
		lockModel: 'dedicated proper-lockfile receipt lock around bounded lookup and transitions; released before knowledge_add production mutation (:866-900)',
		crashBehavior: 'PREPARED is atomically durable before mutation; final failures remain PREPARED or become IN_DOUBT; malformed/truncated history fails closed (:907-1158)',
		closePolicy: 'untouched — terminal history is archived automatically for reconciliation; unresolved records remain in the active journal; archive exhaustion refuses new writes',
		closeArrayMembership: {
			'mcp-write-receipts.jsonl': 'neither',
			'mcp-write-receipts-archive.jsonl': 'neither',
		},
		resetPolicy: 'not reset',
		...
		disposition: {
			kind: 'not-a-defect',
			proof: 'The active journal and terminal archive have independent hard record/byte caps with a 16 KiB per-line cap; terminal records move to the bounded archive automatically while PREPARED and IN_DOUBT records remain active, and both stores are read with fixed bounds (src/mcp/write-receipts.ts:37-45,587-760).',
		},
	},
	{
	...
	},
	{
		...
		writerCitations: [
			...
			'src/knowledge/identity.ts:201 writeProjectIdentity — platform config identity.json',
	...
	// ─────────────────────────────────────────────────────────────────────────
	{
		...
		writerModules: [
			...
			'src/db/qa-gate-session-override.ts',
		...
		writerCitations: [
			'src/db/project-db.ts getProjectDb — canonical-identity cache key, WAL + synchronous NORMAL default + busy_timeout 5000 + foreign_keys; versioned migrations v1-v40 with failed-migration recovery (migration_failures + marker fallback); closeProjectDb runs a best-effort TRUNCATE→PASSIVE WAL checkpoint',
		...
		readerCitations: ['qa-gate profile/session-override/constraint/checkpoint-receipt/insight-candidate/phase-report consumers — indexed SQL via the canonical cached handle'],
		schemaVersion: 'schema_migrations versioned (40; independently exercised by scripts/repro-2487-parity-entry.ts)',
		stateClass: 'authoritative',
		privacyClass: 'metadata',
	...
	{
		id: 'instruction-pairing-report',
		category: 7,
		pathGrammar: '.swarm/memory/instruction-pairing-report.json',
		canonicalRoot: 'project-swarm',
		writerModules: ['src/memory/instruction-pairing.ts'],
		writerCitations: [
			'src/memory/instruction-pairing.ts runInstructionSelectionPairing — whole-file writeFileSync of the finished report (single JSON object, one file per project)',
		],
		readerCitations: [
			'operator/agent reads the report file on demand after /swarm memory evaluate --instruction-pairing; no runtime reader (write-only surface for the plugin)',
		],
		schemaVersion: 'instruction-pairing report v1 (schema_version: 1)',
		stateClass: 'derived-rebuildable',
		privacyClass: 'metadata',
		writeLimits: { bound: 'single file, overwritten whole on each run — bounded by the paired task corpus (deterministic default corpus or caller-supplied task set); no history accumulates', scope: 'per-trigger', citation: 'src/memory/instruction-pairing.ts runInstructionSelectionPairing — writeFileSync replaces the previous report atomically-per-call; no append path' },
		readBound: { pattern: 'write-only', bound: 'n/a (operator-facing output artifact; re-generated by re-running the command)', sync: true, citation: 'src/memory/instruction-pairing.ts report write path' },
		lockModel: 'none (single whole-file overwrite per command invocation)',
		crashBehavior: 'a failed run leaves the previous report untouched (write happens once, after all measurement completes)',
		closePolicy: 'untouched (disposable derived output; safe to delete at any time — re-generated on demand)',
		closeArrayMembership: {},
		resetPolicy: 'not reset',
		legacyCompatibility: 'n/a',
		healthSignal: 'n/a',
		owner: 'this-gate',
		disposition: { kind: 'not-a-defect', proof: 'Derived-rebuildable single-file report overwritten whole per invocation (no append, no history); bounded by the caller-controlled task corpus and fully regenerable by re-running the command.' },
	},
	{
		id: 'documents-cache',
		category: 7,
	...
	},
	{
		...
		writerCitations: ['src/test-impact/analyzer.ts:1109 saveImpactMap — atomic cache write (rebuildable derived cache)'],
		readerCitations: ['src/test-impact/analyzer.ts:1102 loadImpactMap — map-only API; authoritative identity validation/rebuild, with unverified skipRebuild envelope reads'],
	...
	},
	{
		...
		writerCitations: ['src/commands/reset-backup.ts:43 backupSwarmStateBeforeReset — cpSync fail-open; :124 pruneOldResetBackups'],
	...
	},
	{
		...
		writerCitations: ['src/worktree/core.ts:181 writeLaneProfileToDiskReal (fs.promises.writeFile :210); :226 removeLaneProfileFromDiskReal'],
	...
	},
	{
		id: 'full-auto-state',
		category: 8,
		pathGrammar: '.swarm/full-auto-state.json (+ .bak, .lock)',
		canonicalRoot: 'project-swarm',
		writerModules: ['src/full-auto/state.ts'],
		writerCitations: [
			'src/full-auto/state.ts:writePersisted — atomic tmp/fsync/rename with .bak recovery',
			'src/full-auto/state.ts:saveFullAutoRunState, startFullAutoRun, pauseFullAutoRun, disarmFullAutoRun, terminateFullAutoRun, incrementFullAutoCounter, incrementOversightFailureCounter, resetOversightFailureCounter, recordFullAutoDenial, resetFullAutoDenials, nextFullAutoOversightSequence, recordFullAutoOversight, recordFullAutoEscalation, recordFullAutoRecoveryProbe — all route their read-modify-write callbacks through withStateLock',
		],
		readerCitations: [
		...
		owner: '#2575',
		disposition: { kind: 'not-a-defect', proof: 'Focused state-locking tests cover single-process creation, two independent workers retaining both updates, held-lock bounded contention, and configuration/storage taxonomy; production lockSync has no positive retries or proceeding-unlocked fallback.' },
	},
	{
		id: 'restart-reconciliation',
		category: 8,
		pathGrammar: '.swarm/session/restart-reconciliation.json',
		canonicalRoot: 'project-swarm',
		writerModules: ['src/session/restart-reconciliation.ts'],
		writerCitations: [
			'src/session/restart-reconciliation.ts recordInterruptedExecution — bunWrite atomic temp+rename rewrite of the whole bounded list, called only from the rehydrateState populate loop (#2668)',
		],
		readerCitations: [
			'src/session/restart-reconciliation.ts readRestartReconciliation — full-file fail-closed parser (absent/corrupt/partial reads as empty; schema-validated entries only)',
			'operator inspection — documented read-only surface in docs/restart-reconciliation.md (restart/inspect runbook)',
		],
		schemaVersion: 'version 1 (foreign/invalid entries skipped)',
		stateClass: 'authoritative',
		privacyClass: 'metadata',
		directFileExemption: {
			reason: 'Owner-named restart reconciliation (#2668): a bounded operator-visible record of executions interrupted by a process boundary. The whole bounded list is atomically rewritten per append (50-entry FIFO, deduped by sessionId+taskId), so there is no unbounded stream and no torn-read surface; the file is fail-open on write and fail-closed on read.',
			reviewedIssue: 2668,
		},
		writeLimits: { bound: '≤50 entries, FIFO overflow drops the oldest; deduped by (sessionId, taskId) so repeated restarts refresh rather than append', scope: 'global', citation: 'src/session/restart-reconciliation.ts MAX_RESTART_RECONCILIATION_ENTRIES; recordInterruptedExecution' },
		readBound: { pattern: 'full-file', bound: 'single bounded JSON list (≤50 entries)', sync: true, citation: 'src/session/restart-reconciliation.ts readRestartReconciliation' },
		lockModel: 'no lock — single atomic temp+rename rewrite per append; last-writer-wins is safe because every rehydrate re-derives the same owner-named entries and dedupe collapses repeats',
		crashBehavior: 'atomic rename leaves either the previous or the new complete list; corrupt/partial bytes read as empty and the next append heals the file',
		closePolicy: 'cleaned — session/ dir lifecycle (lives under .swarm/session/)',
		closeArrayMembership: {
			'restart-reconciliation.json': 'neither',
		},
		resetPolicy: 'reset-session deletes session/ contents',
		legacyCompatibility: 'n/a (introduced by #2668)',
		healthSignal: 'entries present after an unplanned restart',
		owner: '#2668',
		disposition: { kind: 'not-a-defect', proof: 'Bounded by construction (50-entry FIFO, dedupe) with atomic writes and fail-open/fail-closed error paths pinned by tests/unit/session/restart-policy-reconciliation.test.ts (cap, dedupe, corrupt-read, fail-open-write).' },
	},
	{
	...
	},
	{
		...
		writerCitations: [
			...
			'src/cli/index.ts:643 saveJson — CLI-managed global/plugin config saves (outside .swarm)',
		],
		readerCitations: ['config loader; CLI loadJson (:461)'],
		...
		readBound: { pattern: 'indexed', bound: 'single config files', sync: true, citation: 'src/cli/index.ts:461' },
	...
	{
		id: 'otlp-export-spool',
		category: 3,
		pathGrammar: '.swarm/otlp-export/ (spool.jsonl + state.json; opt-in, absent unless enabled)',
		canonicalRoot: 'project-swarm',
		writerModules: ['src/observability/otlp-exporter.ts'],
		writerCitations: [
			'src/observability/otlp-exporter.ts:358 appendSpoolRecord — byte-capped rewrite with drop-oldest',
			'src/observability/otlp-exporter.ts:322 persistState — throttled atomic state write',
		],
		readerCitations: [
			'src/observability/otlp-exporter.ts:332 readSpoolLines — flush-bounded read, sync',
			'src/observability/otlp-exporter.ts:830 readOtlpExporterHealth — health/report read, sync',
		],
		schemaVersion: 'spool record v1 / state v1 (SPOOL_RECORD_VERSION/STATE_VERSION; mapping versions pinned separately #2029)',
		stateClass: 'operational',
		privacyClass: 'metadata',
		writeLimits: { bound: 'spoolMaxBytes (default 1 MiB, min 16 KiB) drop-oldest + spoolMaxAgeMs (default 24h) age sweep with terminal drop reasons (schema.ts observability.export); MAX_SPOOL_LINES_PER_FLUSH 2048; MAX_FLUSH_ITERATIONS 100', scope: 'global', citation: 'src/observability/otlp-exporter.ts:358-394' },
		readBound: { pattern: 'line-bounded', bound: 'flush reads at most MAX_SPOOL_LINES_PER_FLUSH lines + health stat', sync: true, citation: 'src/observability/otlp-exporter.ts:580-583' },
		lockModel: 'single-flight flush promise (in-process); drop-oldest rewrites are whole-file atomic',
		crashBehavior: 'spool survives crash; unshipped records replay after re-registration (restart replay); shipped-batch removal happens only after a 2xx',
		closePolicy: 'untouched',
		closeArrayMembership: { 'otlp-export': 'neither' },
		resetPolicy: 'not reset',
		legacyCompatibility: 'n/a',
		healthSignal: '/swarm report **OTLP export** section: state, mapping version, spooled records/bytes, accepted/exported/retried/dropped-by-reason, circuitOpen (src/commands/report.ts)',
		owner: '#2485',
		disposition: { kind: 'retain-by-design', citation: 'Opt-in bounded export spool: byte+age budgets with drop-oldest terminal reasons (src/observability/otlp-exporter.ts:358-394), privacy-filtered BEFORE append (no raw payload ever spooled, :773-781), restart replay by design (#2485).' },
	},
	{
		id: 'training-vault',
		category: 3,
		pathGrammar: '.swarm/training/v1/{consent.json,vault/records.jsonl,vault/quarantine.jsonl,tombstones.jsonl,health.json,pending-op.json}',
		canonicalRoot: 'project-swarm',
		...
		readerCitations: [
			...
			'/swarm dataset withdraw — revocation reads (src/commands/dataset.ts)',
		],
		...
		closePolicy: 'withdrawal physically empties records.jsonl, appends a durable tombstone, revokes controlled exports (REVOKED.json)',
		resetPolicy: 'consent revocation stops capture immediately; purge is the confirmed withdraw step',
	...
	},
	{
		id: 'training-exports',
		category: 3,
		pathGrammar: '.swarm/training/v1/exports/<export-id>/{records.jsonl,train.jsonl,validation.jsonl,manifest.json,REVOKED.json}',
		canonicalRoot: 'project-swarm',
		writerModules: ['src/training/exporter.ts'],
		writerCitations: [
			'src/training/exporter.ts:342 (executeTrainingExport — confirm-token gated, deterministic bundle)',
			'src/training/exporter.ts:286 (issueTrainingConfirmToken — single-slot pending-op.json)',
		],
		readerCitations: ['dataset consumers read the manifest checksums; withdrawal writes REVOKED.json (src/training/vault.ts)'],
		schemaVersion: 'export bundle v1 (manifest with sha256 checksums, echoed filters, session-coherent split)',
		stateClass: 'governed-content',
		privacyClass: 'content',
		writeLimits: { bound: 'max 20 exports / 1 GiB total / 30 days (TRAINING_EXPORT_QUOTA_CEILINGS); identical re-export idempotent', scope: 'global', citation: 'src/training/exporter.ts:342 (executeTrainingExport quota gate)' },
		readBound: { pattern: 'indexed', bound: 'deterministic: identical vault + filters produce byte-identical bundles', sync: false, citation: 'buildTrainingExportBundle (canonical sorted-key JSON)' },
		lockModel: 'single-writer per project; atomic file writes',
		crashBehavior: 'bundle files written atomically; a partial bundle is re-exportable idempotently (same export id)',
		closePolicy: 'withdrawal writes a REVOKED.json revocation manifest into every controlled export',
		resetPolicy: 'withdrawn with the vault (tombstone references revoked export ids)',
		legacyCompatibility: 'n/a (new tree)',
		healthSignal: 'training_vault_health (export stop/quota reasons)',
		owner: '#2486',
		disposition: { kind: 'retain-by-design', issue: 2486, citation: 'src/training/exporter.ts:342', note: 'Governed dataset export delivered by #2486 (D7): preview + confirm token, deterministic bundle with manifest checksums and session-coherent split.' },
	},
	{
	...
	},
	{
		...
		canonicalRoot: 'project-swarm',
		writerModules: ['src/services/task-cohort.ts'],
		writerCitations: [
			'src/services/task-cohort.ts:179 persistSnapshotManifest — writeFileSync after mkdirSync (:190), FIFO trim unlinkSync (:212)',
		],
		readerCitations: [
		...
		stateClass: 'derived-rebuildable',
		privacyClass: 'metadata',
		writeLimits: { bound: 'FIFO 20 files (COHORT_MANIFEST_RETENTION); one write per snapshot with a directory; each file is one bounded JSON of the snapshot population', scope: 'global', citation: 'src/services/task-cohort.ts:212' },
		readBound: { pattern: 'indexed', bound: 'single-file reads by exact name; no directory scans beyond the FIFO trim', sync: true, citation: 'src/services/task-cohort.ts:206' },
	...
	},
	{
		...
		readerCitations: [
			'src/background/stage-b-dispatch-binding-store.ts readStageBDispatchBindings — single indexed file, openSync(O_NOFOLLOW)+fstatSync 64KB cap, identity+TTL+per-entry shape validation, fail-closed to null',
		],
		schemaVersion: 'stage-b binding v1 (schemaVersion, sessionID, callID, recordedAt, bindings[])',
		stateClass: 'operational',
		privacyClass: 'metadata',
		writeLimits: {
			bound: 'per-session-dir file cap 128 enforced at every write on the owning dir; 128-session-dir LRU ceiling enforced the moment a write sees it exceeded; empty session dirs are garbage-collected during prune (rmdir only on a truly empty dir — an in-flight .tmp guards it), so the ceiling tracks live dirs and a finished session leaves nothing behind; 24h TTL prune also collects crashed .tmp leftovers; both path components validated single segments (no nesting)',
			scope: 'global',
		...
		resetPolicy: 'reset removes the whole tree recursively (summaries/ pattern, reset.ts); reset-session removes the invoking session’s subtree (path derivation single-sourced from the store module)',
		legacyCompatibility: 'n/a (new tree)',
		healthSignal: 'dispatch-time persistence failures are non-fatal but ALWAYS-VISIBLE (criticalWarn; the in-memory map remains the primary fence)',
		owner: '#2829',
		disposition: { kind: 'retain-by-design', issue: 2829, citation: 'src/background/stage-b-dispatch-binding-store.ts:1', note: 'Durable twin of the Stage B dispatch-generation fencing token (#2817 option (b)): records only what the in-memory map already records; reconstruction feeds the UNCHANGED expectedGeneration fence; bounded by per-dir cap + session-dir LRU (empty dirs GC’d on prune) + TTL.' },
	},
	];
