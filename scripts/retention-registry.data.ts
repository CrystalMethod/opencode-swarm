...
export const RETENTION_REGISTRY: readonly RetentionRow[] = [
	...
	},
	{
		...
		writerCitations: ['src/test-impact/analyzer.ts:534 saveImpactMap — plain writeFileSync :562 (non-atomic; rebuildable cache)'],
		readerCitations: ['src/test-impact/analyzer.ts:479 loadImpactMap — full-file + staleness check (per-file statSync)'],
		schemaVersion: 'impact map schema with generatedAt staleness',
		stateClass: 'derived-rebuildable',
		privacyClass: 'metadata',
		writeLimits: { bound: 'rebuildable via buildImpactMap (:465-471); size bounded by repository file population', scope: 'session-scoped', citation: 'src/test-impact/analyzer.ts:465-471' },
		readBound: { pattern: 'full-file', bound: 'rebuildable cache; stale entries rejected by mtime check', sync: true, citation: 'src/test-impact/analyzer.ts:49-81,479-531' },
		...
		owner: 'this-gate',
		disposition: { kind: 'not-a-defect', proof: 'Derived rebuildable cache with staleness detection; any torn write self-heals on rebuild (analyzer.ts:49-81,465-471).' },
	},
