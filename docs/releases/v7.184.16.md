...
- The delegation store now refuses — at the claim linearization point, under the store lock — an `error` terminal with `workflowLaneFailureClass: "contract"` whose result lacks a receipt the record provably holds (`claimTerminalResult`, src/background/pending-delegations.ts). That signature — a terminal that passed record-result integrity and failed only at the receipt branch — arises from a stale-snapshot decision (the collect settle path is the sole producer of such terminals and always includes its snapshot's receipt in the result when it had one; a "mismatched structured receipt" rejection carries the receipt and is never refused). Degraded/empty content-integrity failures keep their typed contract class and are never refused or admitted. A refused claim leaves the record open; the next collection pass re-reads records at entry and settles the lane on the now-visible receipt.
- When the terminal claim wins the lock outright BEFORE the child's publish lands (the claim-first interleaving), the child's exactly-bound receipt publish is now ADMITTED onto that provably-stale terminal (`publishPrReviewResultReceipt`): the record settles `completed` on the receipt, mirroring the terminal the settle path would have written had its snapshot observed it, and the stale error text and contract class are dropped (the original error text survives only in pre-compaction ledger history and logs). A non-binding receipt still conflicts; `liveness`-class terminals keep the #2585/#2700 parent-repair lever untouched; a replay is a `duplicate`. Every publish-vs-claim ordering of a real receipt now settles on the child's receipt, and the self-contradicting terminal state (missing-receipt error next to a valid receipt in the same record) is unpersistable at any ordering.
