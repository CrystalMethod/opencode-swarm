...
- **Symptom:** strict single-system providers (the v6.85.1 Qwen3.6/Gemma class: "require exactly one `{role:'system'}` message at index 0") received exactly two system messages on every architect turn with guidance enabled, and crashed or silently degraded. The v6.85.1 residual was honest about it: "'exactly one system message at index 0' is not achieved end-to-end and never was."
...
**Per-caller contract table (issue #2674 — the last unbounded git spawn sites on tool/service surfaces; extended by #2705):**
...
| `complexity-hotspots.ts` `getGitChurn` (via the `complexity_hotspots` tool) | `GIT_CHURN_TIMEOUT_MS` = 10 000 ms — caller-owned `Promise.race` deadline **plus** the wrapper-owned tree-kill timer | pipe + `maxBuffer` 5 MiB (`BunCompatOutputLimitError` auto-kill on breach) | `ignore` | explicit `cwd: directory` | `killProcessTree: true` + best-effort `proc.kill()` in `finally`; any kill/timeout/**host abort (#2705: the race honors `ctx.abort` and surfaces a cancellation-specific structured error before the deadline)** surfaces as the tool's structured `error` JSON, never an empty success |
| `build-check.ts` `executeCommand` (via the `build_check` tool; #2705) | `DEFAULT_TIMEOUT_MS` = 300 000 ms (wrapper escalates to SIGKILL on timeout) | pipe + explicit `maxBuffer` = `DEFAULT_BUN_SPAWN_MAX_BUFFER_BYTES` (5 MiB) at the site | `ignore` (#2705 — was the wrapper's silent `mapStdio` default `'pipe'`, the never-closed stdin-pipe class) | explicit `cwd: command.cwd` | best-effort `proc.kill()` in `finally` after output settle (#2705); platform-shell command form justified in-code — discovery emits shell command lines by contract (`cmake -B build && cmake --build build`, `<manager> run <script>`) |
| `pkg-audit.ts` runner family — `runNpmAudit`/`runPipAudit`/`runCargoAudit`/`runGoAudit`/`runDotnetAudit`/`runBundleAudit`/`runDartAudit`/`runComposerAudit` (8 sites; #2705) | caller-owned `Promise.race(AUDIT_TIMEOUT_MS)` = 120 000 ms + `proc.kill()` on the timeout arm (npm/cargo under cmd.exe also `killProcessTree`) | pipe + wrapper-level 5 MiB default (`MAX_OUTPUT_BYTES` slices the retained JSON to 50 MB AFTER capture; the wrapper bound is the memory bound) | `ignore` (#2705 — all 8 sites were the silent `mapStdio` default) | explicit `cwd: directory` | `proc.kill()` on the timeout arm; every failure path returns an `incomplete: true` `AuditResult`, never a false clean |
| `diff.ts` AST helpers `fileExistsInRef`/`getContentFromRef` (#2705) | 3 000 ms / 15 000 ms | pipe + explicit `maxBuffer: MAX_BUFFER_BYTES` (5 MiB, matching the file's numstat/contract/fullDiff siblings; was the implicit 1 MiB sync default) | `ignore` (via `stdio: ['ignore','pipe','pipe']`) | explicit `cwd: directory` | sync `execFileSync` — the timeout IS the kill; per-file AST `try/catch` tolerates the throw (file skipped, not a crash) |
| `identity.ts` legacy `deriveProjectHash` (#2705; deprecated, kept for compile-compat — see #1846) | 1 500 ms | stdout piped + explicit `maxBuffer: GIT_REMOTE_URL_MAX_BUFFER_BYTES` (64 KiB, canonical-sibling parity; was the implicit 1 MiB sync default); stderr ignored | `ignore` | `cwd: directory` + `-C .` | sync `execFileSync`; any failure falls back to the path-derived hash |

Platform notes for these callers:

- **`killSignal: 'SIGKILL'`** (sync callers) is load-bearing on POSIX only — a child that traps SIGTERM must not defeat the bound. On Windows, Node coerces SIGKILL identically to the default SIGTERM handling (an abrupt `TerminateProcess`), so the option is a no-op there. **Runtime caveat (#2705, per the #2674 final critic): the POSIX claim is Node-on-POSIX only.** Bun's sync (`execFileSync`-family) timeout kill has been observed to signal SIGTERM regardless of `killSignal: 'SIGKILL'`, so a SIGTERM-trapping child can survive the bound under Bun-on-POSIX — no caller-side fix exists short of converting the sync calls to async spawns with wrapper-owned kill. This is why the #2674 trap-escalation tests (`identity-subprocess-bounds`, `diagnose-git-repository-subprocess-bounds`) are Windows-gated; cross-OS plain-hang coverage is retained in the churn suite (async wrapper-owned SIGKILL).
- **Explicit `env: { ...process.env, GIT_TERMINAL_PROMPT: '0' }`** (all three callers) does double duty: it prevents the git credential-prompt hang class outright, and it restores live-env semantics under Bun — a Bun `spawn`/`spawnSync` with no `env` option inherits a **process-start env snapshot**, so runtime `process.env` mutations never reach the child (the oven-sh/bun#29237 class documented in `src/utils/git-executable.ts`).
