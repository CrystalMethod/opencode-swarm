...
| `stage-b-dispatch-bindings` | .swarm/stage-b-dispatch-bindings/<sha256(sessionID).slice(0,24)>/<callID>.json | operational | per-session-dir file cap 128 enforced at every write; 128-session-dir LRU ceiling enforced on overflow; empty session dirs GC’d on prune (in-flight .tmp guarded); 24h TTL prune also collects crashed .tmp; both path components validated single segments (global) | indexed: single bounded JSON ≤64KB, identity+TTL+shape-validated fail-closed read | untouched by close — fencing tokens outlive the session they fence; reset clears the tree, reset-session the session subtree | retain-by-design — #2829 |
| `live-lane-owners` | .swarm/live-lane-owners.json | operational | MAX_LIVE_LANE_OWNERS=256 entries + 128 KiB serialized, drop-oldest on both axes (global) | indexed: single bounded JSON read per call plus GC-on-read (lane path gone or 24h claim window passed => reaped) | untouched by close — live lanes must survive; records cleared by removeWorktree or GC | not a defect — #2527 |
| `pending-purge` | .swarm/pending-purge.json | operational | single fixed-shape record, 15-minute TTL, single slot — no accumulation axis (global) | indexed: single small JSON read per validation; expired reads as absent | untouched — a pending confirmation dying with the session is the safe direction | not a defect — #2527 |
| `dead-lane-reclaims` | .swarm/dead-lane-reclaims.json | operational | 512 entries / 256 KiB serialized, drop-oldest on both axes (:36-37,:116-128) (global) | full-file: single ≤256 KiB JSON, read once per record and once per startup reclaim pass | untouched — a pending record must survive close so the next start's init-orphan-recovery reclaims the stranded lane | not a defect — #2599 |
| `workspace-snapshot-digest` | .swarm/workspace-snapshot.digest | derived-rebuildable | single 65-byte advisory marker (one sha256 hex line), atomically overwritten per capture; write skipped entirely when the digest is unchanged (global) | line-bounded: single 65-byte marker read once per capture | untouched — close sweep lands in a sibling fix of the same issue | not a defect — #2472 |
...
| `pr-review-run-artifacts` | .swarm/pr-review/{run_id}/{findings.jsonl, feedback-handoff.json, trigger-eval.json} + .swarm/pr-review/route-receipts/{encoded-session}--{encoded-task}.json or `sha256_<pair-hex>.json` when the complete basename would exceed 255 bytes (each identity is UTF-8/base64url encoded with `-` mapped to `~`; components over 180 characters use deterministic `sha256_<hex>`, and the pair fallback hashes the identity tuple) | governed-content | per-run: findings ≤1000 records/call + 10 MiB read guard; route receipts ≤64 KiB/file; run directories and each route-receipt file age-prune independently at 30 d | line-bounded: 10 MiB findings read guard; route receipts 64 KiB hard read bound | untouched by close — the 30 d sweep owns the run and route-receipt reap | not a defect — #2483 |
| `pr-review-workflow-gate-state` | .swarm/pr-workflow-gates/{session-stem}.json (+ .lock, .imported, .sqlite-projection) + .swarm/swarm.db coordination_state[pr-workflow.state:<session-stem>, state] (SQLite authority; JSON projection; session stem is sanitized slug+digest of sessionID) | authoritative | one bounded coordination_state row and one atomic JSON projection per workflow session; projection sidecars are bounded one-for-one (per-key; keyspace finite by the 30 d reaper) | indexed: one bounded coordination row plus one bounded session projection | Activation takes checkout-mutation then session-state locks; terminal clear takes the session-state lock only. JSON projections/sidecars are untouched by close — the 30 d sweep owns their lifecycle; the SQLite authority follows the swarm.db project-db archive+clean lifecycle; terminal gate deletion removes authority/projection while bounded pr_workflow_* core events remain separate audit history | not a defect — #2510 (historical implementation: #2385) |
...
| `harness-opt-store` | .swarm/evolution/harness-opt/{state.json,tasksets/{taskSetHash}/**,rounds/{roundId}/record.json,pilots/{recordId}/record.json} | authoritative | harness_opt.max_rounds bounds governed rounds (default 5); task-set inputs content-addressed and immutable (global) | directory listing + single-file reads: history bounded to last 20 rounds; replay reads one record | untouched — governed-round lineage and pilot evidence substrate | retain by design — #2503 |
...
| Row id | Path grammar | State class | Write limit (scope) | Read bound | Close policy | Disposition → owner |
|---|---|---|---|---|---|---|
| `plan-ledger` | .swarm/swarm.db plan_ledger_* + portable plan-ledger.jsonl and recovery archives | authoritative | exact append-only history; no sampling or truncation (global); content-addressed legacy archives retain newest 16 and age-prune older than 30 d | JSONL full-file in file-shadow; ordered SQLite rows after cutover | database and portable export archived, then terminal state removed; legacy archives remain for retention (close does not delete them) | retain by design — this-gate; dual-store lifecycle and direct-file shadow/export reviewed in #2484 |
...
| `pr-feedback-loop-state` | .swarm/pr-feedback-loop-state.json (+ .lock) + pr-feedback-evidence/{seq}-{uuid}.json + pr-feedback-loop-cleanups/ | operational | one rewritten state (200-correlation FIFO) + one evidence JSON per oversight dispatch (evidence age-pruned 30 d, pr-feedback-evidence family) + one receipt per cancellation (per-trigger) | full-file: Zod-validated state read | state retained — cross-run idempotency basis (digests/budgets); evidence 30 d age-pruned; close: neither | not a defect — this-gate; direct-file exemption (#2502) |
...
### Category 6 — Knowledge family (12 rows)
...
| `mcp-write-receipts` | .swarm/mcp-write-receipts.jsonl + .swarm/mcp-write-receipts-archive.jsonl | authoritative | active MAX_RECEIPT_RECORDS 500 / 512 KiB + terminal archive MAX_RECEIPT_ARCHIVE_RECORDS 10000 / 16 MiB / 16 KiB line cap (global); terminal records archive automatically and unresolved records remain active | line-bounded: active ≤512 KiB/500 records; archive ≤16 MiB/10000 records | untouched (MCP audit history retained; active/archive capacity fails closed) | not a defect — #2500; direct-file exemption (#2500) |
...
| `full-auto-state` | .swarm/full-auto-state.json (+ .bak, .lock) | authoritative | single rewritten state; denialHistory capped 100; bounded by run lifecycle (global) | indexed: single JSON | untouched (cross-session automation state) | not a defect — #2575; direct-file exemption (#2575) |
| `restart-reconciliation` | .swarm/session/restart-reconciliation.json (#2668) | authoritative | ≤50 entries FIFO, deduped by (sessionId, taskId); whole-list atomic rewrite per append (global) | full-file: single bounded JSON list | cleaned — session/ dir lifecycle (reset-session deletes session/ contents) | not a defect — #2668; direct-file exemption (#2668) |
