...
| live-provider | evidence: the three #2585 live cases re-executed at the rebased closing revision (5e27c2475, plugin build 7.184.11, dist digest 95d3a6f3…) as installed-package journeys through the real OpenCode host (frozen cases + run records + artifact digests under the #2586 issue trace; verifier verify-live-runs-2586.mjs reports ALL CHECKS PASSED, host pin recorded in live-runs-2586/host-declaration.json). Re-execution reason: main advanced past the trace base touching the PR-review owning path, so per the issue's owning-path invalidation contract every live case was rerun rather than carried forward; first-round (pre-rebase) artifacts remain preserved in the trace as prior evidence only. Truthful outcomes at the closing head: case-02 (finding-bearing) REQUEST_CHANGES with COMPLETE coverage 6/6 — the planted stale-citation defect was found and reviewer-confirmed, the critic settled it via its typed skip predicate, and every confirmed finding was routed to action:report by the shipped disposition policy (LOW severity ⇒ no feedback handoff; the verifier's REQUEST_CHANGES⇒handoff assumption was disproved and amended with a disclosure-only admission). case-03 (two planted issues) — both issues were found and reviewer-confirmed with CONFIRMED/DISPROVED/PRE_EXISTING discrimination at the reviewer rung, but the kimi critic lanes stalled to their terminal horizons in two waves (the same provider-availability failure the first round recorded) and the controller terminated via the audited recovery abort: mechanical terminal INCOMPLETE, disclosed. case-01 (clean control) INCOMPLETE with PARTIAL coverage — both explorer dimensions exhausted their bounded retry budgets against defect #2865: an intermittent collect-vs-receipt-publish visibility race on the PR-review collect path, PRE-EXISTING on main (reproduced in both the first-round and re-run build cohorts; the child lanes publish valid exactly-bound structured receipts that the collect-side validation snapshot misses), which turns a lost race into a per-attempt discovery-contract failure because the legacy transcript adapter is disabled by default. The run record carries the frozen-expectation deviation disclosure with met:false obligation rows; #2865 is filed with the ledger forensics. Consequence for this matrix: the clean-control cell is QUALIFIED-WITH-DEFECT — completion of a clean review journey is demonstrated end-to-end except for the disclosed #2865 race cells, and no COMPLETE-terminal clean control is claimed at this revision. |
