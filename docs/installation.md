...
| `compaction` | boolean | `true` | Enrich session compaction with bounded plan/context facts inside a summary-only, no-tools boundary; includes declarative stored-output facts when `.swarm/summaries/` is populated |
