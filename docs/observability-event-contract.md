...
`src/commands/close/archive-stage.ts:38` (`emitCloseArchiveResult`, called after the clean
...
| 12 | `.swarm/archive/swarm-{ts}-{suffix}/` | `src/commands/close/archive-stage.ts:146-153` | `src/commands/close/orchestrator.ts:142-146` (`readdir` + `startsWith('swarm-')` — finalize-idempotency + reflection filename scans only; nothing re-reads bundle *contents*) | n/a | ISO in path | preserves bytes verbatim | n/a | n/a | is the archive | governed content | **#2030** |
